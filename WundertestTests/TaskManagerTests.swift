...
class TaskManagerTests: XCTestCase {
    ...
    func testGetTasks() {
        ...
        XCTAssertGreaterThan(TaskManager.sharedInstance.getTasks(from: self.realm).count, 0, "Should return something from database")
    }
    
    func testCreateTask() {
        XCTAssertEqual(self.realm.objects(Task.self).count, 0, "Should be empty in the database")
        TaskManager.sharedInstance.create(task: Task(), realm: self.realm) // Create a task and save into database
        XCTAssertEqual(TaskManager.sharedInstance.getTasks(from: self.realm).count, 1, "Should return only ONE task from database")
    }
    
    func testGetTask() {
        XCTAssertEqual(self.realm.objects(Task.self).count, 0, "Should be empty in the database")
        TaskManager.sharedInstance.create(task: Task(), realm: self.realm) // Create a task and save into database
        let tasks = TaskManager.sharedInstance.getTasks(from: self.realm) // Should return a new created task from database
        XCTAssertEqual(TaskManager.sharedInstance.getTasks(from: self.realm).count, 1, "Should return only ONE task from database")
    ...
    func testDeleteTask() {
        ...
        XCTAssertEqual(TaskManager.sharedInstance.getTasks(from: self.realm).count, 0, "Should be empty in the database again")
    }
    ...
    func testCompleteTaskAndGetCompletedTasks() {
        ...
        XCTAssertEqual(TaskManager.sharedInstance.getCompletedTasks(from: self.realm).count, 1, "Should return ONE completed task from database")
    }
    
    func testGetIncompletedTasks() {
        XCTAssertEqual(self.realm.objects(Task.self).count, 0, "Should be empty in the database")
        TaskManager.sharedInstance.create(task: Task(), realm: self.realm) // Create a task and save into database
        XCTAssertEqual(TaskManager.sharedInstance.getIncompletedTasks(from: self.realm).count, 1, "Should return ONE incompleted task from database")
    }
    ...
    func testUpdateTaskContent() {
        ...
        let updatedTasks = TaskManager.sharedInstance.getTasks(from: self.realm) // Should return a new updated task from database
        XCTAssertEqual(TaskManager.sharedInstance.getTasks(from: self.realm).count, 1, "Should return only ONE task from database")
