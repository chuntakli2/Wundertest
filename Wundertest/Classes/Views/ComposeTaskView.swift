...
class ComposeTaskView: UIView, UITextViewDelegate, DateTimeRemoveButtonStackViewDelegate, DatePickerViewDelegate {
    ...
    var dueDate: Date? {
        didSet {
            if let date = self.dueDate {
                ...
                var attributes: [NSAttributedStringKey: Any] = ((todayYear >= year && todayMonth >= month && todayDay > day) ? FONT_ATTR_MEDIUM_RED : FONT_ATTR_MEDIUM_DEFAULT_TINT)
                ...
                var attributes: [NSAttributedStringKey: Any] = FONT_ATTR_MEDIUM_DEFAULT_TINT
                attributes[NSAttributedStringKey.underlineStyle] = NSUnderlineStyle.styleSingle.rawValue
    ...
    var reminder: Date? {
        didSet {
            if let date = self.reminder {
                ...
                var attributes: [NSAttributedStringKey: Any] = ((todayYear >= year && todayMonth >= month && todayDay >= day && todayHour >= hour && todayMinute > minute) ? FONT_ATTR_MEDIUM_RED : FONT_ATTR_MEDIUM_DEFAULT_TINT)
                attributes[NSAttributedStringKey.underlineStyle] = NSUnderlineStyle.styleSingle.rawValue
                if let task = self.task {
                    if (task.isCompleted) {
    ...
    private func setupBlurBaseView() {
        self.blurBaseView.effect = UIBlurEffect(style: .extraLight)
    }
    ...
    private func setupBlurTextView() {
        self.blurTextView.effect = UIBlurEffect(style: .extraLight)
    ...
    private func setupBlurSaveButton() {
        self.blurSaveButton.effect = UIBlurEffect(style: .extraLight)
